...
// 			if (b == QMessageBox::No) return;
		...
		this->enableFluidMesh(ok);
		ok = meshOption->isFilterMeshEnabled();
		this->enableFilterMesh(ok);
		ok = meshOption->isMeshModelingEnabled();
		this->enableMeshModeling(ok);
		_ui->SetToolBar->setVisible(ok || meshOption->isComponentEnabled() || meshOption->getCheckMeshEnabled() ||
		meshOption->isFilterMeshEnabled() || geometryOption->isGeometryCreateSetEnabled());
...
//		qDebug() << logo;
	...
	void CustomizerHelper::enableGeometryEdit(bool on)
	{
		_ui->menuGeometry_Edit->setEnabled(on);
		_ui->GeometryEditToolBar->setVisible(on);
	}

	void CustomizerHelper::enableGeoCreateSet(bool on)
	{
		_ui->actionCreateGeoComponent->setVisible(on);
	}

	void CustomizerHelper::enableMeasureDistance(bool on)
	{
		_ui->actionMeasure_Distance->setVisible(on);
		_ui->MeasureToolBar->setVisible(on);
	}
	...
	void CustomizerHelper::enableFluidMesh(bool on)
	{
		_ui->actionFluidMesh->setVisible(on);
	}

	void CustomizerHelper::enableFilterMesh(bool on)
	{
		_ui->actionFilterMesh->setVisible(on);
	}

	void CustomizerHelper::enableMeshModeling(bool on)
	{
		_ui->actionVTKTranslation->setVisible(on);
	}
